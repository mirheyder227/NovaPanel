...
const storage = new CloudinaryStorage({
  ...
  params: {
    ...
    public_id: (req, file) =>
      `product-${Date.now()}-${file.originalname.split('.')[0].replace(/[^a-zA-Z0-9-_]/g, '')}`,
    resource_type: 'auto'
  }
});
...
export const deleteImageFromCloudinary = async (imageUrl) => {
  ...
  try {
    ...
    if (result.result === 'ok') {
      ...
      console.warn(`Cloudinary-dən şəkil silinərkən xəta: ${result.result}`);
    }
  } catch (error) {
    console.error(`Cloudinary-dən şəkil silinərkən xəta:`, error);
  }
};
